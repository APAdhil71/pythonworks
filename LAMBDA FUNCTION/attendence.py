...
srt_attendence=sorted(attendance,key=lambda dict:dict.get("attendance_count"),reverse=True)
# print(srt_attendence)

name_attendence=[{st.get("name"):st.get ("attendance_count")} for st in srt_attendence]
print(name_attendence)

srt_hacker_summary=[{st.get("name"):st.get("count")} for st in attendance]
print(srt_hacker_summary)